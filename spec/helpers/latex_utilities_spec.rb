...
describe LatexUtilities do
  ...
  describe '#conv_pm' do
    ...
    it '' do
      exp = add('x',2,-3,4,5)
      expect(dummy_class.conv_pm(exp)).to eq add(sbt(add('x',2),3),4,5)
    end
    ...
    it '' do
      exp = '3(x-4)-2(x-5)'.objectify
      expect(dummy_class.conv_pm(exp)).to eq sbt(mtp(3,sbt('x',4)),mtp(2,sbt('x',5)))
    end
