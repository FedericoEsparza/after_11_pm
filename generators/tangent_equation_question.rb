class TangentEquationQuestion
  RS_VALUES = [frac(sqrt(3), 3), sqrt(3), 0, 1]
  ...
  def evaluate_numerals
    ...
    ((Math.atan(rs_var).degrees.round - b.to_f) / a_var)
  end
  ...
  def compose_equation
    ...
    rs_var =  if rs[:value].is_a?(Fraction) || rs[:value].is_a?(square_root)
    ...
    tan_eqn(ls, rs_var, options: {ans_min: limits[0], ans_max: limits[1]})
  end
